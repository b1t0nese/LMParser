...
class StaticMapAPI:
    ...
    def move_to_location(self, toponym: str, map_scale: int=None) -> tuple[float, float, str] | None:
        ...
        if not map_scale:
            self.map_scale = {
                "entrance": 20,
                "house": 18, "metro": 18, "railway_station": 18, "airport": 18, "station": 18, "route": 18,
            ...
            self.map_scale = map_scale
        return self.longitude, self.latitude, loc_metadata["Address"]["formatted"]
...
class WindowUI(QMainWindow):
    def initUI(self):
        ...
        self.choice_theme.setFixedHeight(40)
...
class TheyMaps(WindowUI):
    ...
    def on_search(self):
        point = self.static_map_api.move_to_location(self.search_entry.toPlainText())
        if point:
