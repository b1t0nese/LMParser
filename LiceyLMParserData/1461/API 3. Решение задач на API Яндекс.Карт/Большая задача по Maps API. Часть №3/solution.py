...
class TheyMaps(QMainWindow):
    def __init__(self, latitude: float, longitude: float, map_scale: int, move_step: float=0.01, logging: bool=False):
        super().__init__()
    ...
    def get_image(self):
        map_request = f"""http://static-maps.yandex.ru/1.x/?ll={self.longitude},{self.latitude}&z={
            self.map_scale}&size={self.width()},{self.height()}&l=map"""
