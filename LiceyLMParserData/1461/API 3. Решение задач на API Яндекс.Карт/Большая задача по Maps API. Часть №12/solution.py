...
def get_address_by_coords(longitude: float, latitude: float, logging: bool=False) -> tuple[str, str] | None:
    api_url, params = "http://geocode-maps.yandex.ru/1.x/", {
        "apikey": GEOCODE_MAPS_API_KEY,
        "geocode": f"{longitude},{latitude}",
        "format": "json"
    }
    response = requests.get(api_url, params)
    if logging:
        print(f"""\nReverse geocode request: {api_url}, {params}\nHttp status: {response.status_code} ({
            response.reason})\nResponse: {response.text}""")
    if not response:
        return
    json_response = response.json()
    ...
    if feature_member:
        ...
        loc_address = geo_object["metaDataProperty"]["GeocoderMetaData"]["Address"]
        print(loc_address)
        return loc_address["formatted"], loc_address.get("postal_code", "")


def find_nearest_org(lon: float, lat: float, logging: bool = False) -> tuple[str, str] | None:
    api_url, params = "https://search-maps.yandex.ru/v1/", {
        "apikey": SEARCH_MAPS_API_KEY,
        "text": f"{lat},{lon}",
        "ll": f"{lat},{lon}",
        "lang": "ru_RU",
        # "type": "biz" ПОЧЕМУ-ТО ОН НИЧЕГО НЕ МОЖЕТ НАЙТИ ИЗ ОРГАНИЗАЦИЙ
    }
    response = requests.get(api_url, params)
    if logging:
        print(f"""\nOrganization search request: {api_url}, {params}\nHttp status: {response.status_code} ({
            response.reason})\nResponse: {response.text}""")
    if not response:
        return None
    ...
    for feature in features:
        org_lon, org_lat = feature["geometry"]["coordinates"][0], feature["geometry"]["coordinates"][1]
        dist = haversine(lon, lat, org_lon, org_lat)
        if dist <= 50 and dist < min_dist:
            min_dist, nearest = dist, feature
    if nearest is None:
        return None

    address = nearest["properties"].get("CompanyMetaData", {}).get("address", "")
    if not address:
        address = nearest["properties"].get("name", "") or nearest["properties"].get("description", "")
    return address, get_address_by_coords(lon, lat, logging)[1]


def haversine(lon1, lat1, lon2, lat2):
    phi1, phi2, dphi = math.radians(lat1), math.radians(lat2), math.radians(lat2 - lat1)
    a = math.sin(dphi/2)**2 + math.cos(phi1) * math.cos(phi2) * math.sin(math.radians(lon2 - lon1)/2)**2
    return 6371000 * 2 * math.atan2(math.sqrt(a), math.sqrt(1-a))


def pixel_to_geo(click_x: float, click_y: float, pixmap_width: float,
                    pixmap_height: float, label_width: float, label_height: float,
                    center_lon: float, center_lat: float, zoom: int) -> tuple[float, float]:
    offset_x, offset_y = (label_width - pixmap_width) / 2, (label_height - pixmap_height) / 2
    world_size = 256 * (2 ** zoom)
    lat_rad = center_lat * math.pi / 180.0
    x_center = (center_lon + 180.0) / 360.0 * world_size
    y_center = (1.0 - math.log(math.tan(lat_rad) + 1.0 / math.cos(lat_rad)) / math.pi) / 2.0 * world_size
    x_click = x_center + click_x - offset_x - pixmap_width / 2
    y_click = y_center + click_y - offset_y - pixmap_height / 2
    lon = x_click / world_size * 360.0 - 180.0
    lat = math.atan(math.sinh(math.pi * (1.0 - 2.0 * y_click / world_size))) * 180.0 / math.pi
    return lon, lat
...
class TheyMaps(WindowUI):
    ...
    def image_mousePressEvent(self, event: QEvent):
        ...
        if not (offset_x <= click_x <= offset_x + pixmap_width and 
                offset_y <= click_y <= offset_y + pixmap_height):
            self.image_focus()
            super().mousePressEvent(event)
            return

        lon, lat = pixel_to_geo(
            click_x, click_y, pixmap_width, pixmap_height,
            label_width, label_height, self.static_map_api.longitude,
            self.static_map_api.latitude, self.static_map_api.map_scale)

        if event.button() == Qt.MouseButton.LeftButton:
            if self.points_button.isChecked():
                self.static_map_api.add_point(lon, lat)
            address_info = get_address_by_coords(lon, lat, self.static_map_api.logging)
            if address_info:
                address, postal_code = address_info
                if self.index_button.isChecked() and postal_code:
                    full_address = f"{address}, {postal_code}"
                else:
                    full_address = address
                self.search_entry.setText(full_address)
            self.update_map()
            self.image_focus()
            event.accept()
            ...
            self.static_map_api.pop_point()
            self.search_entry.clear()
            org_info = find_nearest_org(lon, lat, self.static_map_api.logging)
            if org_info:
                address, postal = org_info
                if self.index_button.isChecked() and postal:
                    full_address = f"{address}, {postal}"
                else:
                    full_address = address
                self.search_entry.setText(full_address)
                if self.points_button.isChecked():
